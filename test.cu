...
__global__
void flash_tiled_coarse_causal(float *out, float *K, float *Q, float* V, float scaling, int batch_stride, int T_r, int T_c, int seq_len)
{
  int tid_x = threadIdx.x;
  int tid_y = threadIdx.y;
  int batch_offset = batch_stride * blockIdx.x;
...
int main() {
  ...
  int batch_size = 8;
  float *K_d, *Q_d, *V_d, *O;
  cudaMalloc((void**)&O, batch_size * seq_len * d * sizeof(float));
