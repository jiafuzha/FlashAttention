    ...
    */
    ...
    for (int j = 0; j < T_c && j <= blockIdx.y ; j++) { // iterate of ver the chunks of K and V
        ...
        for (int t=0; t<num_tiles; t++){
            ...
            //  for (int i=0; i<(B_c*BK); i+=numThreadsBlocktile){
            //     int id = j * B_c * d + t * BK + threadId_flat + i;
            //     uint innerRowK = (threadId_flat + i) / BK;
            //     uint innerColK = (threadId_flat + i) % BK;
            //     if (id < d*seq_len){
        ...
        for (int ii=0;ii<TM;ii++){
            for (int jj=0;jj<TN;jj++){
                if(blockIdx.y*B_r+threadRow*TM+ii < seq_len){
                    out[batch_offset + (blockIdx.y * B_r + threadRow*TM + ii) * d + t * BK + threadCol*TN + jj] = O_i[t*TN*TM+ii*TN+jj] / l_i[ii];
                }
            }
        } 
    }
