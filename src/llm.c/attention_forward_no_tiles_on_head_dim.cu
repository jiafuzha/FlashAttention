...
{   // used by attention_forward6
    ...
    */
    ...
    for (int t=0; t<(B_r * d); t+=numThreadsBlocktile){
      ...
      id = (blockIdx.y * B_r + innerRowQ) * d + innerColQ + t;
      // 4 x 4 then this is 5 thus 5/
      if (id < d*seq_len){
        Q_i[innerRowQ][innerColQ + t] = Q[batch_offset + id];
      }
      else {
        Q_i[innerRowQ][innerColQ + t] = 0.0;
      }
    }
    ...
    for (int j = 0; j < T_c && j <= blockIdx.y ; j++) { // iterate of ver the chunks of K and V
        ...
        for (int t=0; t<num_tiles; t++){
            ...
            for (int i=0; i<B_r; i+=strideK){
                ...
                // if (not CACHE_Q){
                //     Q_i[innerRowK+i][innerColK] = Q[batch_offset + (innerRowK + blockIdx.y * B_r) * d  + i * d + innerColK + t * B_c];
                // } // if you cache Q over whole d then remove this line
                id = (innerRowK + j * B_c) * d + i * d + innerColK + t * BK;
                if (id < d*seq_len){
...
{   // used by attention_forward6
    ...
    */
    ...
    __shared__ float K_j[B_c][d+1]; // reduce SMEM bank conflicts by adding 1 column as K will be loaded transposed!
    ...
    //WARNING: due to coalsecing I should probably add a second set of variables for using BK+1
    const uint strideK = numThreadsBlocktile / d; // 64 / 64 = 1
    uint innerRowK = threadId_flat / d; // 0-63 / 64, 0000000000000...0
    uint innerColK = threadId_flat % d; // 0-63 % 64, 0123456789101112...63
    ...
    for (int j = 0; j < T_c && j <= blockIdx.y ; j++) { // iterate of ver the chunks of K and V
        ...
        // for (int t=0; t<num_tiles; t++){
            // load K_j and V_j, thread idx, idy loads idy,idx
            // we load a tile
            for (int i=0; i<(B_c*d); i+=numThreadsBlocktile){
                int id = j * B_c * d + threadId_flat + i;
                uint innerRowK = (threadId_flat + i) / d;
                uint innerColK = (threadId_flat + i) % d;
                if (id < d*seq_len){
                    K_j[innerRowK][innerColK] = K[batch_offset + id];
                    //V_j[innerRowK+i][innerColK+t*B_c] = V[batch_offset + id];
                } else {
                    K_j[innerRowK][innerColK] = 0.0;
                    //V_j[innerRowK+i][innerColK+t*B_c] = 0.0;
                }
            ...
            // for (int dd=0; dd<BK; dd++){ // load elements of Q_i and K_j^T into registers
            for (int dd=0; dd<d; dd++){ // load elements of Q_i and K_j^T into registers
                for (uint i = 0; i < TM; ++i) {
                    regM[i] = Q_i[(threadRow * TM + i)][dd]; // uncomment if you cache Q over full d
                }
                for (uint i = 0; i < TN; ++i) {
                    regN[i] = K_j[threadCol * TN + i][dd];
                }
...
void attention_forward7(float* out,
    ...
    double start, end;
    start = getTimeStamp();
    flashattention2<<<gridDim, blockDim>>>(out, k, q, v, softmax_scale, (N+B_r-1)/B_r, (N+B_c-1)/B_c, N);
    cudaDeviceSynchronize();
    end = getTimeStamp();
